...
const initialData: NotificationItem[] = [
  {
    ...
    category: "Payments",
    status: "unread",
    sender: "GymLink System",
    message: "Payment received for Plan Gold - $49.99",
    time: "Just now",
    icon: "cash-fast",
  },
  {
    ...
    message: "Membership renewal due in 3 days for John D.",
    time: "5 min ago",
    icon: "refresh",
  },
  {
    ...
    message: "New Zumba class added to the weekly schedule.",
    time: "2 hours ago",
    icon: "bullhorn",
  },
  {
    ...
    status: "read",
    sender: "System Admin",
    message: "Payout initiated to your bank account.",
    time: "Yesterday",
    icon: "bank-transfer",
  },
];
...
export default function NotificationsScreen() {
    ...
    return (
      <Pressable
        ...
        className={`px-3 py-2 rounded-full mr-2 ${
          isActive
            ? ""
            : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-gray-800"
        }`}
        style={{
      ...
      >
        <Text
          className={`text-xs ${
            isActive ? "text-white" : "text-slate-700 dark:text-gray-300"
          }`}
        >
          {label}
        </Text>
  ...
  return (
    <ScreenWrapper title="Notifications" theme={theme}>
              ...
              >
                ...
                <View className="flex-1">
                  <View className="flex-row items-center justify-between">
                    <Text className="text-slate-900 dark:text-gray-100 font-semibold text-sm">
                      {n.sender}
                    </Text>
                    <Badge status={n.status} />
                  </View>
                  <Text className="text-slate-700 dark:text-gray-300 text-xs mt-1">
                    {n.message}
                  </Text>
                  <View className="flex-row items-center justify-between mt-2">
                    ...
                    <View className="flex-row items-center">
                      ...
                      >
                        <Text className="text-[12px]" style={{ color: accent }}>
                          View Details
                        </Text>
                      </Pressable>
                      {n.status === "unread" && (
                        <Pressable onPress={() => markAsRead(n.id)}>
                          <Text className="text-[12px] text-emerald-600 dark:text-emerald-400">
                            Mark as Read
                          </Text>
                        </Pressable>
                      )}
