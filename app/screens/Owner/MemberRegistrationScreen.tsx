...
export default function MemberRegistrationScreen() {
  const [paymentStatus, setPaymentStatus] = useState<
    "Paid" | "Pending" | "Advance"
  >("Paid");
  const { theme, accentColor } = useAppTheme();
  const accent = accentColor || (theme === "dark" ? "#4EA1FF" : "#1d74f5");

  return (
    <ScreenWrapper title="Register New Member" theme={theme}>
      <View className="flex-1 bg-white dark:bg-slate-950">
        <ScrollView contentContainerStyle={{ paddingBottom: 120 }}>
          {/* Form */}
          <View className="px-4 mt-3">
            <View
              className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-gray-800"
              style={{
                shadowColor: "#000",
            ...
            >
              ...
              <Pressable className="border border-slate-200 dark:border-gray-800 rounded-lg px-3 py-3 flex-row items-center justify-between mb-4 bg-white dark:bg-slate-950">
                <Text className="text-slate-900 dark:text-gray-100">
                  Select a gym/branch
                </Text>
                <IconMC
                  name="menu-down"
                ...
                {(["Paid", "Pending", "Advance"] as const).map((s) => (
                  <Pressable
                    ...
                    className={`px-4 py-2 rounded-full border ${
                      paymentStatus === s
                        ? "border-2"
                        : "bg-white dark:bg-slate-950 border-slate-200 dark:border-gray-800"
                    }`}
                    style={
                      paymentStatus === s
              ...
              {/* Submit */}
              <Pressable
                className="rounded-xl py-3 px-4 items-center justify-center mt-1"
