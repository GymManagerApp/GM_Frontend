...
export default function StaffRegistrationScreen() {
  ...
  const Chip = ({ label }: { label: "Active" | "Pending" | "Inactive" }) => {
    ...
    const colors: Record<string, { bg: string; text: string; border: string }> = {
      ...
      Inactive: { bg: "bg-rose-50", text: "text-rose-700", border: "border-rose-200" },
    };
    const c = colors[label];
    return (
      <Pressable
        onPress={() => setStatus(label)}
        className={`px-3 py-2 rounded-full border ${c.border} ${active ? c.bg : "bg-white"}`}
      >
    ...
    <View className="flex-1 bg-white">
      <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
        ...
        <View className="px-4 mt-2">
          ...
          >
            <Text className="text-slate-900 font-semibold mb-3">Staff Details</Text>
            ...
            <Pressable className="border border-slate-200 rounded-lg px-3 py-3 flex-row items-center justify-between mb-4">
              <Text className={`text-slate-900 ${role ? "" : "text-slate-400"}`}>{role || "Select a role"}</Text>
            ...
            <Pressable className="border border-slate-200 rounded-lg px-3 py-3 flex-row items-center justify-between mb-4">
              <Text className={`text-slate-900 ${gym ? "" : "text-slate-400"}`}>{gym || "Select a gym"}</Text>
              <IconMC name="chevron-down" size={18} color="#94a3b8" />
            </Pressable>

            {/* Employment Status Chips */}
            <Text className="text-slate-700 text-sm mb-2">Employment Status</Text>
